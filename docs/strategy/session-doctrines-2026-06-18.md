...
12. **Safe-by-construction, precision-deferred.** Ship the mechanism that is safe by its very *shape* first; let precision ratchet in async. *Worked example (D3 lock, 2026-06-22):* the Tier-3 disambiguation engine **forces** the operator to pick the source (so it can never silently mis-stamp an allergen) and renders a generic honest marker ("verify per source — COA") — safe with zero sign-off. Co-founder ratification upgrades marker *precision* (exact mollusk string, fish species sub-set, kelp triple-marker wording), not *safety*. Same shape as the audit-ratchet for catalog quality. **Integration:** in scoping, distinguish **ship-gates** from **precision-gates** — a precision-pending item never blocks a safe-by-construction ship. *(Proposed — raised at the D3 lock; future-work for the rulebook pass.)*

11. **Narrow therapeutic window is a first-class honesty marker.** A small RDA→UL ratio means accidental over-dose is a live harm — surface it alongside allergens and heavy-metals vectors, not buried in dosing. *Worked example (C1 form-set session, 2026-06-22):* Iodine (RDA 150 / UL 1100 mcg = 7.3×) combined with kelp's order-of-magnitude supplier variability = compounding over-dose risk → the kelp triple-marker (COA + As vector + therapeutic-window warning) is the honest answer. Generalizes: Selenium (55/400 = 7.3×), Vitamin A retinol, copper. **Integration:** a Class-4 marker the engine surfaces at compliance-packet time + a safety-engine high-dose flag. *(Proposed — raised mid-C1; future-work for the rulebook integration pass.)*
