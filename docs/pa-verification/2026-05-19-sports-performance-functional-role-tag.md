...
3. **Re-tagging of existing entries:** three entries currently use the tag (two identified during inaugural validator smoke test 2026-05-19; one added during step-5 inaugural exercise 2026-05-19):
   ...
   - `Caffeine from Green Tea Extract (50% Caffeine, USP Identity)` — added step 5 entry 3 (2026-05-19); same caffeine active as Caffeine Anhydrous USP, plant-source delivery form, source-independent ergogenic substantiation per IOC Maughan 2018 (caffeine is the active regardless of source). PA decision needed: does Caffeine from Green Tea Extract inherit the parent Caffeine Anhydrous sports-performance substantiation automatically (source-independent), or does it need a separate substantiation evaluation? Recommend: inherit. The 50% plant-source extract delivers the same caffeine molecule at the same UL/dose envelope.

4. **`cognitive-support` Appendix A enumeration (added during step-5 entry-3 verdict):** Appendix A enumerates `cognitive-support` with Bacopa 300 mg + Phosphatidylserine 100 mg as substance examples. Caffeine 50-200 mg attention-effect is substantively defensible (Haskell 2008 / Owen 2008) but not listed. Should Appendix A's `cognitive-support` row enumerate caffeine 50-200 mg as a substance example? Applies uniformly to both Caffeine Anhydrous and Caffeine from Green Tea Extract.
5. **Threshold expression format:** Appendix A's existing rows use specific dose thresholds (`adaptogenic: ≥ 200 mg standardized extract`; `nootropic: clinical cognitive effect at typical dose`). The `sports-performance` threshold is substance-dependent (3-6 mg/kg for caffeine; 3-6 g/day for beta-alanine; etc.). Should Appendix A express this as a per-substance threshold ladder, or as a generic "≥ ergogenic dose per Tier-1 sports-nutrition authority"?
