...
12. **F&B catalog meat-ingredient gap + bacon-subtype processMethod deferral.** Section 3b.2's meat-basis denominator routing requires `isMeat: true` ingredients in the formulation for cured-meat regulations (Prague Powders, Morton Tender Quick, Phosphates (meat), Erythorbate/Ascorbate, Binders dairy/soy) to compute correct percent/ppm. The current F&B catalog focuses on acidified foods and condiments; whole-cut meats (beef trim, pork shoulder, chicken thigh, fatty/lean grades, poultry vs red meat) live in the sausage / cured-meat vertical mode's catalog or are absent entirely. v1 ships a single synthetic catalog entry `Generic Lean Meat (Test Fixture)` to enable test coverage of meat-basis denominator routing against the live catalog. Round 11+ catalog expansion should add proper meat-category coverage with cut-specific entries (lean / regular / fatty grades with realistic protein/fat profiles) so cured-meat formulators can compose accurate productions. **Separately:** sodium nitrite's bacon-subtype caps (120 ppm pumped / 200 ppm immersion-cured / 250 ppm dry-cured per 9 CFR 424.21) are NOT routed by productClass alone — they require a `processMethod` field on the formulation (cure-method selection). v1 implements `appliesToCategories: ['cured-meat', 'bacon']` + `contextualLimits: [{ context: 'bacon', maxPpm: 120 }]` — bacon productClass routes to the strictest (pumped, 120 ppm) cap as a conservative default. Immersion-cured and dry-cured bacon products will be over-flagged until Round 11+ adds processMethod-aware routing (`'bacon-pumped'` / `'bacon-immersion'` / `'bacon-dry'` contextualLimits contexts paired with a processMethod field on the formulation). Over-flagging is the safe direction: customer-zero verifies their cure method against the engine's conservative default; pumped-bacon products pass cleanly.

13. **Section 3c entries pending PA verification.** The four directive-specified new entries (LAE, natamycin, sodium diacetate, nisin) plus the open-ended common-e-numbers enumeration require PA-verified citations + caps + scope before they can ship in `REGULATORY_LIMITS`. CC does NOT pull values from training data and tag them as authoritative — the directive's "PA review on the citations and caps before commit" language locks this discipline. Pending items are queued in [docs/pa-verification/](../pa-verification/) — one file per substance with the fields PA needs to verify, landing location, and open questions. Section 3d (Bucket A enforcement gate) is composable over whatever entries exist in `REGULATORY_LIMITS` and lands independently; Section 3c lands when PA returns verified values. Round 10 ships with the current 18 entries + 3b.1/3b.2 corrections + Section 3d's gate — the four new entries land via follow-up commit once unblocked.
