...
- **Current implementation status:** **Wired (Phase 1 Step 4 + Phase 2 Step 4 follow-up migration, 2026-05-17).** Constants + frozen-snapshot test + claim-count selector landed at [`lib/supplementDisclaimer.ts`](../../lib/supplementDisclaimer.ts) + [`lib/__tests__/supplement-disclaimer.test.ts`](../../lib/__tests__/supplement-disclaimer.test.ts). Gate registration at [`lib/supplementBucket1Gate.ts`](../../lib/supplementBucket1Gate.ts) `COMPOSED_ITEMS`. Phase 2 Step 4 follow-up migration: [`buildDisclaimers()`](../../lib/supplementClaims.ts) signature changed from `hasStructureFunctionClaim: boolean` to `claimCount: number`; inline string literal replaced with `selectSupplementDisclaimer(claimCount)` routing — fixes the pre-migration regression where the function always emitted PLURAL regardless of claim count. Asterisk-footnote prefix (`'* '`) preserved as the presentational convention for label rendering. Caller at [`app/workspace/page.tsx:5503`](../../app/workspace/page.tsx#L5503) updated to pass `sfClaims.length` directly. Tests at [`supplement-build-disclaimers.test.ts`](../../lib/__tests__/supplement-build-disclaimers.test.ts) (12 cases): claim-count routing 0/1/2/5/100/negative + singular/plural subject verification + FDA anchor preservation + additionalWarnings continuity across migration. Remaining gaps:
