...
- **Current implementation status:** **Wired (Phase 1 Step 4 + Phase 2 Step 4 follow-up + Phase 3 Workstream A.5 SFP closure, 2026-05-17).** Constants + frozen-snapshot test + claim-count selector landed at [`lib/supplementDisclaimer.ts`](../../lib/supplementDisclaimer.ts) + [`lib/__tests__/supplement-disclaimer.test.ts`](../../lib/__tests__/supplement-disclaimer.test.ts). Gate registration at [`lib/supplementBucket1Gate.ts`](../../lib/supplementBucket1Gate.ts) `COMPOSED_ITEMS`. Phase 2 Step 4 follow-up migration: [`buildDisclaimers()`](../../lib/supplementClaims.ts) consumes `selectSupplementDisclaimer(claimCount)`; Claims Validator card caller at [`app/workspace/page.tsx`](../../app/workspace/page.tsx) updated. Tests at [`supplement-build-disclaimers.test.ts`](../../lib/__tests__/supplement-build-disclaimers.test.ts) (12 cases). **Phase 3 Workstream A.5 closure of Phase 2 implementation-discovery finding #10:** second rendering site at the Supplement Facts Panel renderer (`app/workspace/page.tsx` lines ~5160-5260, inline JSX) was emitting a hardcoded plural disclaimer unconditionally — independent rendering site that the Phase 2 migration didn't reach. Migrated to consume `selectSupplementDisclaimer(claimCount)` with `detectStructureFunctionClaims(ingredients.map(i => i.name)).length` computed at the SFP IIFE; conditional render guards the empty-string case (0 claims → no disclaimer per CFR 101.93(c)). Asterisk-footnote prefix preserved as SFP presentational convention. Closure verified by full vitest suite (542/542) + type-check + the existing supplement-disclaimer.test.ts frozen-snapshot change-control trail. Remaining gaps:
